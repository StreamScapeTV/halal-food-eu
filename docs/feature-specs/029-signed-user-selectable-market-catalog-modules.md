...
- **HF-MODULE-015:** The module manifest binds at minimum: module schema/version ID, market code, catalog/schema/methodology versions, app compatibility range, database byte count and SHA-256, inner catalog-manifest SHA-256, attribution/license bundle digest, product/unique-GTIN/ingredient-observation/assessment/retailer-evidence counts, quality/coverage/freshness summary, source snapshot/release identity, compressed/installed bytes, publication timestamp distinct from evidence dates, signing key ID, and supersedes/rollback identity.
- **HF-MODULE-016:** Before activation the app verifies the bundled trust policy, key ID/trust state, canonical closed-schema module manifest, detached Ed25519 signature, database/manifest/license digests and byte bounds, exact market, schema/application IDs, app compatibility, source-policy identity, SQLite integrity/counts, and the existing production catalog/search compatibility checks. Every persisted activation repeats verification under the current bundled policy. `active` keys may authorize new installs, `retired` keys only preserve already-installed verification during rotation, and `revoked` key/module/database identities are unusable. A failure at any step rejects the candidate before it becomes queryable.
- **HF-MODULE-017:** Downloads use an isolated temporary location with fixed filenames, regular non-symlink file/directory checks, bounded redirects/hosts/bytes/time/concurrency, and are atomically promoted into a versioned Application Support directory only after full verification. Downloaded module files are reproducible public data and are excluded from device cloud backup. The previous verified module is retained until replacement succeeds. Crash, cancellation, truncated/tampered data, GitHub outage, rate limiting, low storage, or incompatible metadata leaves the last verified module/bundled baseline usable.
...
- **HF-MODULE-024:** Only the reviewed manual protected-`main` module-release workflow may sign/publish a market module, and it may consume only successful `catalog-release.yml` **production** evidence for the same exact current `main` SHA. The private Ed25519 seed is available only in the protected `catalog-module-release` environment and must derive to an `active` public key already present in the bundled trust policy before signing. Initial distribution uses one immutable market-specific GitHub Release/tag identity per module version. It is published as a prerelease, every fixed asset is re-downloaded through the public HTTPS release URL and independently revalidated, a durable release report is attached, and only then may the release become stable. A failed verification may delete only the prerelease/tag created by that exact run. A CDN/object-store path requires demonstrated GitHub scale/rate-limit need and a later accepted delivery change; no custom backend is introduced merely for hosting.
- **HF-MODULE-025:** Ordinary push and pull-request validation remains fully available on GitHub-hosted runners without Central CI, release credentials, GitHub Release publication, or network product data. Deterministic production-shaped fixtures exercise the signing/artifact verifier and application seam, while a two-market same-GTIN fixture proves routing isolation. Test private/public keys are fixture-only and must never be accepted by the shipping trust policy.
...
- manifest count/coverage/freshness fields remaining exact and never converting a partial/discovered corpus into retailer completeness; and
